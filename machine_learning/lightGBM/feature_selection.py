...
class FeatureSelection():
    ...
    def interpret(self):
        ...
        model = LGBMClassifier(
            boosting_type='gbdt', importance_type='split', random_state=2019)
        lgb = LGBMClassifier(
            boosting_type='gbdt', importance_type='split', random_state=2019)
        ...
        # dump(model, 'lgb_model_union.joblib')
        lgb.fit(train_X_new, train_y)
        dump(lgb, 'lgb_model_plus.joblib')
        model = load('lgb_model_union.joblib')
        # lgb = load('lgb_model_plus.joblib')
        pred = model.predict_proba(eval_X).tolist()
        pred_new = lgb.predict_proba(eval_X_new).tolist()
        pred_y = model.predict(eval_X).tolist()
        pred_y_new = lgb.predict(eval_X_new).tolist()
        ...
        for idx in range(test_size):
            ...
            if label != pred_y_new[idx] and label == pred_y[idx]:
                print(label, pred_y[idx], pred_y_new[idx], pred[idx],
                      pred_new[idx])
                """
                instance = eval_X.loc[[idx]]
                _, bias, contributions = ti.predict(model, instance)
                print("Bias", bias)
                print("Feature contributions:")
                for c, feature in zip(contributions[0], feature_set):
                    print(feature, c)
                """
            diff.append(abs(pred[idx][label] - pred_new[idx][label]))
    ...
    def feature_elimination(self):
        lgb = LGBMClassifier(
            boosting_type='gbdt', importance_type='split', random_state=2019)
        rfecv = RFECV(lgb, step=1, cv=10, scoring='f1', n_jobs=3)
